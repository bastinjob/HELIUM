...
public:
  explicit PrintOpLowering(mlir::MLIRContext *context)
    : mlir::ConversionPattern(Helium::PrintOp::getOperationName(), 1, context) {}
  ...
  mlir::LogicalResult matchAndRewrite(mlir::Operation *op,
    ...
    auto printOp = mlir::cast<Helium::PrintOp>(op);
...
namespace {
class HeliumToLLVMLoweringPass
        : public mlir::PassWrapper<HeliumToLLVMLoweringPass, mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(HeliumToLLVMLoweringPass)
...
void HeliumToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<Helium::PrintOpLowering>(&getContext());
...
std::unique_ptr<mlir::Pass> Helium::createLowerToLLVMPass() {
  return std::make_unique<HeliumToLLVMLoweringPass>();
}